    ...
    try {
        ...
        const agg = [
            ...
            {$group: {
                ...
                avgCompletionsMs:{$avg: {$subtract: ["$completedAt", "$createdAt"]}},
                tasks:{$push: {id:"$_id", title:"$title", completedAt:"$completedAt"}}
            }},
            {$lookup:{
                from:"users",
                localField:"_id",
            ...
            {$project:{
                ...
                email:"$user.email",
                tasksCompleted:1,
                avgCompletionHours:{$round:[{$divide:["$avgCompletionMs", 1000*60*60]},2]},
                tasks:1
            }},
        ...
        if((req.query.format || "").toLowerCase() === "csv"){
            ...
            res.attachment(`productivity_${start.toISOString().slice(0,10)}_${end.toISOString().slice(0,10)}.csv`)
            return res.send(csv)
        }
  ...
  try {
    ...
    const agg = [
      ...
      { $group: {
          ...
          entries: { $push: { id: "$_id", user: "$user", project: "$project", duration: "$duration", startTime: "$startTime", endTime: "$endTime" } }
      }},
    ];
    ...
    if (groupBy === "user") {
      ...
      if ((req.query.format || "").toLowerCase() === "csv") {
        ...
        res.header("Content-Type", "text/csv");
        res.attachment(`timesheet_user_${start.toISOString().slice(0,10)}_${end.toISOString().slice(0,10)}.csv`);
        return res.send(csv);
      }
      ...
      if ((req.query.format || "").toLowerCase() === "csv") {
        ...
        const csv = parser.parse(out);
        res.header("Content-Type", "text/csv");
        res.attachment(`timesheet_project_${start.toISOString().slice(0,10)}_${end.toISOString().slice(0,10)}.csv`);
        return res.send(csv);
      }
      return res.json({ success: true, data: out });
    }
